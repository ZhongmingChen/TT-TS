...
I = [500 500 500]; % Tensor size
K = 10; % Sketch dimension parameter
J1 = 100; % First sketch dimension
% J2 = K*prod(R); % Second sketch dimension
noise_level = 0; % Amount of noise added to nonzero elements
% tol = 1e-6; % Tolerance
% maxiters = 50; % Maximum number of iterations
nnzY = 1e+3; % Approximate number of nonzeros in tensor

%% Generate random dense tensor
% fprintf('Generating random dense tensor... ');
% Y = tensor(randn(I)) + noise_level*randn(I);
% fprintf('Done!\n\n');
...
% fprintf('\nRunning tucker_ts...\n\n')
% tt_ts_tic = tic;
% [G, err] = tt_ts(Y, R, J1, 'tol', tol, 'maxiters', maxiters, 'verbose', true);
% tt_ts_toc = toc(tt_ts_tic);
%% Run tucker_ts
