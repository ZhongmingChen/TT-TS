function [X, cores] = generate_low_rank_ttensor(sz, R, noise, varargin)
%generate_low_rank_tensor Generate dense (TT) low-rank tensor
%
...
%     R1 = ranks(n);
    cores{n} = randn(R(n), sz(n), R(n+1));
%     if large_elem > 0
%         r0 = randsample(R(n+1), 1);
%         i = randsample(sz(n), 1);
%         r1 = randsample(R(n), 1);
